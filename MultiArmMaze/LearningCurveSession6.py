...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  1 10:26:17 2019
...
fig_name = figDirPath()+ 'MultiMazeFigures/' + 'Session6.pdf'

sourceDir = data_folder / 'MultiMazeData/session6/'
...
#    datetime_object = datetime.datetime.strptime(PosFile[20:-7], '%Y-%m-%d %I.%M.%S')
    ...
    nan_to_valz = np.interp(t[nan_ind],t[~nan_ind],z[~nan_ind])
    
    x[nan_ind], z[nan_ind]=  nan_to_valx, nan_to_valz
    ...
    Thresh = np.where(radi > 0.5)[0]
    angle = np.arctan2(z,x)
    
    angle2= angle[Thresh]
    
    hist_angle, angles = np.histogram(angle2,np.linspace(-np.pi,np.pi,60))
    
    arm_angle_id, arm_info = sg.find_peaks(hist_angle,threshold=50)
    arm_angle = np.sort((angles[arm_angle_id])*(180/np.pi))
    
    Reward1, Reward2 = arm_angle[3], arm_angle[6]
...
#    time_incorr_choice = time_true[np.where(reward_logic==0)]
    ...
    for ang1 in range(0,len(angle_degree)):
        ...
        if (sum(run_avg)==0):
            ...
            if abs(deg-Reward1)<10 or abs(deg-Reward2)<10:
                run_avg[ang1] = 1
                temp = deg
                
        if (sum(run_avg)!=0):
            
            if abs(deg-Reward1)<10 and abs(temp-deg)>10:
                run_avg[ang1] = 1
                temp=deg
                
            if abs(deg-Reward2)<10 and abs(temp-deg)>10:
                run_avg[ang1] = 1
                temp=deg
...
#    plt.plot(ti, true_sensor_t,'m',label = 'Sensor reward')
    plt.plot(run_avg_t,percent_correct,label=sub_name,color= colmap[sub], linewidth = 2, alpha=0.95-sub/10, linestyle= '-')
    plt.ylim(0, 1.1)
