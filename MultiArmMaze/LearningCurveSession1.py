...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 27 10:53:27 2019
...
fig_name = figDirPath() + 'MultiMazeFigures/' + 'Session1.pdf'

sourceDir = data_folder / 'MultiMazeData/session1/'
...
#    plt.plot(ti, true_sensor_t,'m',label = 'Sensor reward')
    ...
    if len(percent_correct) > 1:
        ...
        plt.plot(run_avg_t, percent_correct, label=sub_name,
                 color=colmap[sub], linewidth=3, alpha=0.95-sub/10, linestyle='-')
    ...
    recordTime = len(x[~np.isnan(x)])/120
    print(recordTime)
