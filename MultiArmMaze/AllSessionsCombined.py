...
#    Allbehav = pd.read_csv(sourceDir / sess_name)
    ...
    for sub in range(0,num_sub):
        ...
        if len(percent_correct)>1:
            ...
            plt.plot(run_avg_t,percent_correct,label=sub_name,color= colmap[sub], linewidth =1.5, alpha=0.95-sub/10, linestyle= '-')
...
#        plt.plot(run_avg_t,percent_correct,label=sub_name,color= colmap[sub], linewidth = 2, alpha=0.95-sub/10, linestyle= '-')
    ...
    plt.plot([0,50],[chanceLevel, chanceLevel],label= 'Chance',color='#a1a7af', linestyle='--', linewidth=1.5)
