...
for sub in range(0, 19):
    ...
    if len(area) > 1:
        ...
        hpcSpindle = np.transpose(
            fspindle['spindle'][sub_name]['HPC']['peakTime'][:])
...
#        diffTime = ((hpcRipple - pfcSpindle).ravel())/1e6
        ...
        if 'Steve' in sub_name:
            ...
            if 'Sleep' in sub_name:
                ...
                plt.plot(edge[0:len(hist)], hist, color='#f49e42',
                         alpha=0.95 - int(sub_name[-1]) / 10, label='Light' + sub_name[-1])

            else:

                plt.plot(edge[0:len(hist)], hist, color='#726f6b',
                         alpha=0.95 - int(sub_name[-1]) / 10)
        ...
        if 'Ted' in sub_name:
            ...
            if 'Sleep' in sub_name:
                ...
                plt.plot(edge[0:len(hist)], hist, color=colmapDark[int(
                    sub_name[-1])], alpha=1, label='Dark' + sub_name[-1])
